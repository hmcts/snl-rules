...
import checks.CsrfCheck.{csrfParameter, csrfTemplate}
import checks.CsrfCheckForPayment.{csrfParameterForPayment, csrfTemplateForPayment}
import checks.CurrentPageUrl.currentPageTemplate
import checks.PaymentSessionToken.{chargeIdParameter, chargeIdTemplate}
import checks.{CsrfCheck, CsrfCheckForPayment, CurrentPageUrl, PaymentSessionToken}

class probateJouneyToPayment extends Simulation {
...
val chain_3 = pause(20)
		...
		.exec(http("PROBATE_180_APPLICANT_05")
			...
			.headers(headers_69)
			.formParam("_csrf", "")
			.formParam("phoneNumber", "07853989215")
			.check(status.is(200)))
...
val chain_8 = exec(http("PROBATE_410_CONTENT_ICON_IMPORTANT")
...
val chain_9 = pause(527 milliseconds)
...
//  		.check(status.is(200)))
  ...
  .exec(http("PROBATE_640_PAYMENT_BREAKDOWN_02")
	...
	val scn = scenario("probateJouneyToPayment").exec(
		chain_0, chain_2, chain_3, chain_4, chain_5, chain_6, chain_7, chain_8, chain_9, chain_10, chain_11, chain_12)
